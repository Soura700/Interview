  ...
  fetchAssigned() {
    ...
    this.http.get<any[]>("http://localhost:5147/api/interviewer/secure/assigned", {
      ...
      next: (res) => {
        ...
        // 🔥 SAME LOGIC AS CandidateHome → Convert resumePath to full URL
        const updated = res.map(a => ({
          ...a,
          candidate: {
            ...a.candidate,
            resumeUrl: a.candidate.resumePath
              ? "http://localhost:5147" + a.candidate.resumePath
              : null
          }
        }));
